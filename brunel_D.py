...
D_values = np.arange(1.0, 10.5, 0.5) * ms  # Изменение D (задержка)
...
for D in D_values:
    ...
    for n_test in tests:
        print('n_test', n_test)
        rate_monitor, spike_monitor, max_rate = sim(g, nu_ext_over_nu_thr, sim_time, epsilon, J, D)
...
plt.errorbar(D_values/ms, mean_rates, yerr=std_rates, fmt='o', capsize=5, label='Average max rate with std deviation')
plt.ylabel('Rate (Hz)')
plt.xlabel('D (ms)')
plt.title(f'g={g}, J={J}, epsilon={epsilon}, nu_ext_over_nu_thr={nu_ext_over_nu_thr}')
plt.tight_layout()
plt.legend()
plt.show()
